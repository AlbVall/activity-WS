<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Contact | Bloodstrike</title>
    @vite(['resources/js/app.js', 'resources/css/app.css'])
</head>
<body class="bg-gray-900 text-white">
   
    <nav class="flex justify-between items-center p-6 bg-gray-800 shadow-lg">
        <h1 class="text-2xl font-bold">Bloodstrike</h1>
        <ul class="flex space-x-6">
            <li><a href="Home" class="text-gray-300 hover:text-white transition">Home</a></li>
            <li><a href="about" class="text-gray-300 hover:text-white transition">About</a></li>
            <li><a href="#" class="text-white font-bold">Contact</a></li>
        </ul>
    </nav>
    
   
    <section class="flex flex-col items-center justify-center h-screen text-center px-4">
        <h2 class="text-5xl font-extrabold mb-4">Contact Us</h2>
        <p class="text-lg text-gray-400 max-w-2xl">Have questions or need support? Reach out to us at:</p>
        <p class="mt-4 text-lg font-semibold text-white">📧 albert.vallente@hcdc.edu.ph</p>
    </section>
</body>
</html>
